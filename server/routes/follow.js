...
router.post('/check-following', async(req, res)=>{
    ...
    try {
        const data = await sql`SELECT * FROM public.friends WHERE following_id = ${friends_id} AND followed_id = ${followed_id}`
        return res.status(200).json(data)
    } catch(error){
...
router.post('/check-follower', async(req, res)=>{
    ...
    try {
        const data = await sql`SELECT * FROM public.friends WHERE followed_id = ${friends_id} AND following_id = ${followed_id}`
        return res.status(200).json(data)
    } catch(error){
...
router.post('/get_follower', async(req, res)=>{
    ...
    try {
        const data = await sql`SELECT * FROM public.follwed_info WHERE following_id = ${user_id}`
        if(data.length === 0){
